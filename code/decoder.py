...
class Decoder(nn.Module):
    """
    Class uses the output of Encoder as an input parameters of the LSTM to predict tags for each word. Depending on the mode,
    uses either labels or previously predicted grammemes as inputs for the next step.
    ...
    def forward(self, labels_batch, decoder_hidden, decoder_cell):
        """
        Takes batches of hidden and cell states produced by Encoder as the initial states for the LSTM. At each timestep
        feeds either predicted grammeme embedding or known labels as the next inputs. Also, perfomes reduction after the prediction to
        match the number of grammems in the vocabuary. Returns predictions (outputs at each timestep) and probabilities that will
        be used to calculate the loss.

        Parameters
        ----------
        labels_batch : torch.Tensor
            Contains labels for each batch. When in train mode, labels are fed as inputs at each timestep. When in test mode,
            only SOS tokens are used.
        decoder_hidden : torch.Tensor
            The initial hidden state for the decoder. Generated by Encoder. Shape (max_sentence_length * batch_size, grammeme_LSTM_hidden)
        decoder_cell : torch.Tensor
            The initial cell state for the decoder. Generated by Encoder. Shape (max_sentence_length * batch_size, grammeme_LSTM_hidden)
        Returns
        -------
        tuple
